...
async function buscarPorId(req, res, next) {
  try {
    const empresa = await prisma.empresa.findUniqueOrThrow({
      ...
      include: { vagas: { select: { id: true, titulo: true, area: true, status: true, cargaHoraria: true, remuneracao: true } } }
    })
    res.json(empresa)
  } catch (err) { next(err) }
}
...
async function atualizar(req, res, next) {
  try {
    ...
    const empresa = await prisma.empresa.update({ where: { id: Number(req.params.id) }, data: dados })
    res.json(empresa)
  } catch (err) { next(err) }
}
